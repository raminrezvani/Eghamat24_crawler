...
def create_cookie_with_selenium():
    ...
    with open("Booking_readyTour_cookies.json", "w") as file:
...
#== for first time ===
if os.path.exists('Booking_readyTour_cookies.json'):
    ''
else:  # file nist
...
def delete_cookies_periodically():
    while True:
        ...
        if os.path.exists('Booking_readyTour_cookies.json'):
            os.remove('Booking_readyTour_cookies.json')
...
def City_dic(source,distination):
    cityDic={}
    cityDic['MHD']='1640810'
    cityDic['THR']='1641221'
    cityDic['KIH']='1640809'
    ...
    try:
        source_tour=cityDic[source]
    except:
        source_tour=''
    try:
        target_tour=cityDic[distination]
    except:
        target_tour=''


    return source_tour,target_tour


def get_booking_tours(source,target,start_date, night_count,adults):
    try:
        start_date = datetime.strptime(start_date, "%Y-%m-%d") + timedelta(days=1)
        end_date = (start_date + timedelta(days=night_count)).strftime("%Y-%m-%d")
        start_date = start_date.strftime("%Y-%m-%d")
        adults=str(adults)
        source_tour,target_tour=City_dic(source,target)
        url = f"https://www.booking.ir/trip/?i={start_date}&o={end_date}&r=1&n=&d={target_tour}&or={source_tour}&a={adults}&c=0#/"
        ...
        while True:
            ...
            if req.status_code != 200:
                print(f'Booking Error in Cookie --- Status_Code: {req.status_code}')
...
def load_cookies():
    if os.path.exists('Booking_readyTour_cookies.json'):
        with open('Booking_readyTour_cookies.json', 'r') as json_file:
            return json.load(json_file)
    else:
        print(' Booking_readyTour_cookies.json not exists !!!!!! ')
        # return renew_and_save_cookies()

# def renew_and_save_cookies():
#     cookies = get_authorization()
#     cookies_dict = {cookie.name: cookie.value for cookie in cookies}
#     with open('Booking_readyTour_cookies.json', 'w') as json_file:
...
def booking_tours():
    ...
    future = executor.submit(get_booking_tours, source,target,start_date, night_count,adults)
